-- Add real per-user SofLIA Dialogue active time (soflia_dialogue_sessions.active_seconds,
-- see 20260710090000_soflia_dialogue_active_seconds.sql) as a priority tier in the admin
-- user-stat RPCs, between real tracking/progress minutes and the static estimated-minutes
-- fallback. This mirrors the fix already applied to the API-side fallback logic
-- (resolveStudyMinutes in apps/web/src/app/api/admin/user-stats/study-time.ts) and to the
-- business-panel analytics aggregator (calculateStudyMinutes), so all "tiempo de
-- aprendizaje" surfaces agree: two users who complete the same SofLIA Dialogue activity at
-- different real paces now get different, accurate minute counts instead of the same
-- admin-configured estimate.
