...
- **Actualización**: los datos nuevos se guardan en tablas de *staging* (`courses_staging`,
...
await supabase.from('courses_staging')
    .update({ status: 'rejected', rejection_reason: reason, reviewed_at: now, reviewed_by: adminId })
    .eq('id', stagingId)
