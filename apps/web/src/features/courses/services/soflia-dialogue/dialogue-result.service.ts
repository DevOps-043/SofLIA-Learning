...
function readPersistedDialogueScore(responsePayload: unknown) {
...
async function syncDialogueResultToActivitySubmission(input: {
  ...
  const client = input.client as Parameters<
    typeof persistActivitySubmissionPayload
  >[0]
  ...
  if (existingSubmission?.status === 'validated') {
    const persistedScore = readPersistedDialogueScore(
      existingSubmission.response_payload,
    )

    if (
      input.result.activityResult !== 'completed' ||
      (persistedScore !== null && persistedScore > input.result.score)
    ) {
      return
    }
  }
