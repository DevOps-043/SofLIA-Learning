...
import { buildActivitySubmissionSummaryMap } from './activity-submission-summary-map.server'
import type {
  ActivityLikeRecord,
  CourseLessonContext,
...
function findLastActivitySubmissionAt(
...
export async function computeLessonActivityProgress(
  ...
  const summaryMap = await buildActivitySubmissionSummaryMap(supabase, context, interactiveActivities)
  ...
  const activityProgressPercentage =
    requiredActivitiesTotal === 0
      ? 100
      : Math.round((requiredActivitiesCompleted / requiredActivitiesTotal) * 100 * 100) / 100

  return {
    activityProgressPercentage,
    lastActivitySubmissionAt: findLastActivitySubmissionAt((submissions || []) as Array<{
