...
export function findOrderedLessonIndex(
  ...
  return lessonId
    ? orderedLessons.findIndex((item) => item.lesson.lesson_id === lessonId)
    : -1
}
...
export function findOrderedLessonById(
  ...
  return orderedLessons.find((item) => item.lesson.lesson_id === lessonId) ?? null
}
...
export function getPreviousOrderedLesson(
  ...
  return currentIndex > 0 ? orderedLessons[currentIndex - 1]?.lesson ?? null : null
}
...
export function canCompleteOrderedLesson(
  ...
  return lessonIndex === 0 || Boolean(orderedLessons[lessonIndex - 1]?.lesson.is_completed)
}
