...
describe('buildNotebookTree', () => {
  ...
  it('attaches compendium rows at course level and counts them', () => {
    const compendium: CompendiumNoteRow = {
      note_id: 'comp1',
      note_title: 'Compendio SofLIA: Alpha',
      note_tags: ['SofLIA', 'Compendio'],
    ...
    expect(tree.totalNotes).toBe(2)
    expect(tree.courses[0].totalNotes).toBe(2)
    expect(tree.courses[0].compendium?.noteId).toBe('comp1')
    expect(tree.courses[0].compendium?.source).toBe('course_compendium')
  })

  it('creates the course node when only a compendium exists', () => {
    const compendium: CompendiumNoteRow = {
      note_id: 'comp1',
      note_title: 'Compendio SofLIA: Beta',
      note_tags: [],
      source_type: 'course_compendium',
      is_auto_generated: true,
      created_at: null,
      updated_at: null,
      course_id: 'c9',
      courses: { id: 'c9', title: 'Beta', slug: 'beta' },
    }
    const tree = buildNotebookTree([], [compendium])

    expect(tree.courses).toHaveLength(1)
    expect(tree.courses[0].courseId).toBe('c9')
    expect(tree.courses[0].lessons).toHaveLength(0)
    expect(tree.courses[0].compendium?.noteId).toBe('comp1')
  })

  it('skips compendium rows without a course relation', () => {
    const orphanCompendium: CompendiumNoteRow = {
      note_id: 'comp1',
      note_title: 'Orphan',
      note_tags: [],
      source_type: 'course_compendium',
    ...
    const tree = buildNotebookTree([], [orphanCompendium])

    expect(tree.totalNotes).toBe(0)
...
describe('toNoteDetail', () => {
  ...
  it('resolves the course through the direct join for compendium notes', () => {
    const row: DetailRow = {
      note_id: 'comp1',
      note_title: 'Compendio SofLIA: Alpha',
      note_content: '<h2>Síntesis</h2>',
      note_tags: [],
      source_type: 'course_compendium',
      is_auto_generated: true,
      created_at: null,
      ...
      organization_id: 'org1',
      course_lessons: null,
      compendium_course: { id: 'c1', title: 'Alpha', slug: 'alpha' },
    }
