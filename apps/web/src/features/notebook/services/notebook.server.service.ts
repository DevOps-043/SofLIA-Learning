...
const DETAIL_SELECT = `
  note_id, note_title, note_content, note_tags, source_type, is_auto_generated, is_user_edited,
...
export async function fetchNotebookNote(params: {
  ...
  // El compendio se recompone en vivo (síntesis + notas por lección) SALVO que
  // el usuario lo haya editado: entonces se respeta su versión guardada tal cual.
  if (
...
export async function updateNotebookNote(params: {
  ...
  const isCompendium = existing.source_type === 'course_compendium'
  // El compendio no tiene lesson_id (agrupa el curso completo); las demás notas sí.
  if (!isCompendium && !existing.lesson_id) {
    throw new NotebookError('Nota no encontrada.', 404)
  }

  // Auto-notas y compendio son generados por SofLIA: al editarlos se marcan como
  // is_user_edited para que el worker (y la recomposición en vivo del compendio)
  // respeten la versión del usuario y no la sobrescriban.
  const isGenerated =
    existing.is_auto_generated || isCompendium ||
  ...
  if (isGenerated) {
    updateData.is_user_edited = true
  }

  // El scope del filtro varía: el compendio se acota por curso (sin lesson_id);
  // el resto por lección. Ambos quedan siempre acotados por usuario y org.
  ...
  updateQuery = isCompendium
    ? updateQuery.eq('source_type', 'course_compendium')
    : updateQuery
        .eq('lesson_id', existing.lesson_id as string)
        .in('source_type', ['manual', 'chat', 'import', 'lesson_auto_note'])

  const { data, error } = await updateQuery.select('note_id').maybeSingle()
