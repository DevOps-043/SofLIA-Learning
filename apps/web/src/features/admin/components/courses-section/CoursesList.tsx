...
function UnifiedOrgCard({
      ...
      >
        <div className="aspect-video relative overflow-hidden bg-black/20">
          ...
          <div className="absolute top-4 left-4">
            <span className="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1 border"
              style={{ backgroundColor: `color-mix(in srgb, ${colors.accent} 15%, #080808)`, color: colors.accent, borderColor: `color-mix(in srgb, ${colors.accent} 30%, transparent)` }}>
        ...
        <div className="p-6 flex-1 flex flex-col">
          ...
          <h5 className="text-base font-bold text-white leading-tight line-clamp-2 mb-4">{data.courses.title}</h5>

          <div className="mt-auto space-y-4 pt-4 border-t border-white/5">
            <div className="flex items-center gap-2">
              <Clock className="w-3.5 h-3.5" style={{ color: colors.grayMedium }} />
              <span className="text-[11px] font-medium" style={{ color: colors.grayMedium }}>
            ...
            {pendingRevoke ? (
              <div className="p-2 bg-red-500/10 border border-red-500/30 rounded-xl">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setPendingRevoke(false)} className="flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-white/10 text-white/60 hover:bg-white/5 transition-all">{tc('actions.cancel')}</button>
                  <button onClick={() => { setPendingRevoke(false); onRemoveCourse(data.course_id) }} className="flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all">{tc('actions.confirm')}</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setPendingRevoke(true)} className="w-full py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-500/10 transition-all opacity-40 hover:opacity-100">
                {t('coursesSection.revoke')}
              </button>
            )}
          </div>
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="group relative rounded-[2rem] overflow-hidden border transition-all hover:shadow-2xl flex flex-col h-full"
      style={{ backgroundColor: colors.bgTertiary, borderColor: 'rgba(255,255,255,0.05)' }}
    >
      <div
        className="aspect-video relative flex items-center justify-center overflow-hidden"
        style={{ background: `linear-gradient(135deg, color-mix(in srgb, ${colors.purple} 35%, #080808), color-mix(in srgb, ${colors.accent} 15%, #080808))` }}
      >
        <Route className="w-14 h-14 opacity-20 transition-all duration-700 group-hover:scale-110 group-hover:opacity-35" style={{ color: colors.purple }} />
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
        <div className="absolute top-4 left-4">
          <span className="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1 border"
            style={{ backgroundColor: `color-mix(in srgb, ${colors.purple} 15%, #080808)`, color: colors.purple, borderColor: `color-mix(in srgb, ${colors.purple} 30%, transparent)` }}>
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <h5 className="text-base font-bold text-white leading-tight line-clamp-2 mb-2">
          {data.learning_path?.title || t('coursesSection.untitledPath')}
        </h5>
        {data.learning_path?.description && (
          <p className="text-sm line-clamp-2 mb-2" style={{ color: colors.grayMedium }}>
            {data.learning_path.description}
          </p>
        )}
        ...
        <div className="mt-auto space-y-4 pt-4 border-t border-white/5">
          ...
          {pendingRevoke ? (
            <div className="p-2 bg-red-500/10 border border-red-500/30 rounded-xl">
              ...
              <div className="flex gap-2">
                <button onClick={() => setPendingRevoke(false)} className="flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-white/10 text-white/60 hover:bg-white/5 transition-all">{tc('actions.cancel')}</button>
                <button onClick={() => { setPendingRevoke(false); onRemovePath(data.id) }} className="flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all">{tc('actions.confirm')}</button>
              </div>
            </div>
          ) : (
            <button onClick={() => setPendingRevoke(true)} className="w-full py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-500/10 transition-all opacity-40 hover:opacity-100">
...
export function UnifiedOrgGrid({ items, onRemoveCourse, onRemovePath }: UnifiedOrgGridProps) {
  const { t } = useTranslation('admin')
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {items.length === 0 ? (
    ...
    <div className="overflow-hidden rounded-3xl border" style={{ backgroundColor: colors.bgTertiary, borderColor: 'rgba(255,255,255,0.05)' }}>
      <table className="w-full text-left">
        <thead style={{ backgroundColor: 'rgba(255,255,255,0.02)' }}>
          <tr>
            <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style={{ color: colors.grayMedium }}>{t('coursesSection.user')}</th>
            <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style={{ color: colors.grayMedium }}>Contenido</th>
            <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style={{ color: colors.grayMedium }}>Tipo</th>
            <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-center" style={{ color: colors.grayMedium }}>{t('coursesSection.progress')}</th>
            <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-right" style={{ color: colors.grayMedium }}>{t('coursesSection.actionsHeader')}</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-white/5">
          {items.length === 0 ? (
            <tr>
              <td colSpan={5} className="px-6 py-20 text-center text-sm" style={{ color: colors.grayMedium }}>
            ...
            items.map(item => {
              ...
              return (
                <tr key={`${item.kind}-${id}`} className="hover:bg-white/[0.01] transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-[10px] font-bold">
                      ...
                      <div>
                        ...
                        <p className="text-[10px]" style={{ color: colors.grayMedium }}>{user?.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col items-center gap-1.5 min-w-[100px]">
                      {item.kind === 'course' ? (
                        ...
                        <span className="text-[10px] font-semibold" style={{ color: colors.grayMedium }}>—</span>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    {item.kind === 'course' ? (
                      pendingRevokeId === id ? (
                        <div className="flex items-center justify-end gap-1">
                          <span className="text-xs text-red-400 mr-1">{t('coursesSection.confirmRevokeUser')}</span>
                          <button onClick={() => setPendingRevokeId(null)} className="px-2 py-1 text-xs border border-white/10 text-white/60 rounded hover:bg-white/5 transition-colors">{tc('actions.cancel')}</button>
                          <button onClick={() => { setPendingRevokeId(null); onRemoveCourse(id) }} className="px-2 py-1 text-xs bg-red-500/20 text-red-400 rounded hover:bg-red-500/30 transition-colors">{tc('actions.confirm')}</button>
                        </div>
                      ) : (
                        <button onClick={() => setPendingRevokeId(id)} className="p-2 rounded-xl hover:bg-red-500/10 text-red-400 opacity-40 hover:opacity-100 transition-all">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      )
                    ) : (
