...
export function AddUserModal({ isOpen, onClose, onSave }: AddUserModalProps) {
  ...
  const tabs: { id: TabType; label: string; icon: typeof UserIcon }[] = [
    { id: 'basic', label: t('admin:users.addModal.tabs.basic'), icon: UserIcon },
    { id: 'personal', label: t('admin:users.addModal.tabs.personal'), icon: UserIcon },
                ...
                <div className="relative bg-gradient-to-r from-[#0A2540] to-[#0A2540]/90 dark:from-[#0A2540] dark:to-[#0A2540]/80 px-6 py-4 border-b border-[#0A2540]/20">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-white">{t('admin:users.addModal.title')}</h3>
                        <p className="text-xs text-white/70">{t('admin:users.addModal.subtitle')}</p>
                      </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
                  ...
                  <div className="px-6 py-4 bg-[#E9ECEF]/30 dark:bg-[#0A0D12] border-t border-[#E9ECEF] dark:border-[#6C757D]/30 flex items-center justify-end gap-3">
                    ...
                    <motion.button type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={isLoading}
                      ...
                      {isLoading ? (
                        <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div><span>{t('admin:users.addModal.creating')}</span></>
                      ) : (
                        <><PlusIcon className="h-4 w-4" /><span>{t('admin:users.addModal.create')}</span></>
                      )}
                    </motion.button>
                  </div>
                </form>
