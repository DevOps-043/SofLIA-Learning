...
export function AdminCompanyMemberRow(props: AdminCompanyMemberRowProps) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl px-3 py-2" style={{ backgroundColor: theme.cardBg }}>
      <div className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl font-extrabold" style={{ backgroundColor: `${theme.primaryColor}14`, color: theme.primaryColor }}>
      ...
      <span className="rounded-xl px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wider" style={{ backgroundColor: `${theme.primaryColor}14`, color: theme.primaryColor }}>{props.roleLabel}</span>
    </div>
  )
}
