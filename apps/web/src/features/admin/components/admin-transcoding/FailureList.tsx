...
export function FailureList({ failures }: { failures: DispatchFailure[] | undefined }) {
  ...
  return (
    <div className="mt-2 rounded-lg bg-error/10 border border-error/30 p-2 text-xs">
      <p className="font-semibold text-error mb-1">
        {failures.length} {failures.length === 1 ? 'disparo fallo' : 'disparos fallaron'}:
      </p>
      <ul className="text-primary dark:text-white/80 space-y-0.5">
        {failures.map(failure => (
          <li key={failure.jobId}>
            ...
            {failure.detail && <span className="text-gray-500 dark:text-white/50"> - {failure.detail}</span>}
          </li>
        ))}
      </ul>
    </div>
