    ...
    <div className={`grid gap-4 ${columnsClass}`}>
      {items.map((metric, index) => {
        ...
        return (
          ...
          >
            <div
              className={`mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br ${metric.gradient}`}
            >
            ...
            <div
              className={`text-xs font-semibold uppercase tracking-wide ${COURSE_MANAGEMENT_MUTED_TEXT_CLASS}`}
            >
              {metric.label}
            </div>
            {metric.sublabel ? (
...
export function CourseManagementStudentDetailsModal() {
  ...
  const weeklyProgress =
    studentDetailsData?.studySessions?.weeklyProgress?.length
      ? studentDetailsData.studySessions.weeklyProgress
      : DEFAULT_WEEKLY_PROGRESS
  const dailyStudyTime =
    studentDetailsData?.studySessions?.dailyStudyTime?.length
      ? studentDetailsData.studySessions.dailyStudyTime
      : DEFAULT_DAILY_STUDY_TIME
  const preferredTimeSlots =
    studentDetailsData?.studySessions?.preferredTimeSlots?.length
      ? studentDetailsData.studySessions.preferredTimeSlots
      : DEFAULT_PREFERRED_TIME_SLOTS
  const activeDays =
    studentDetailsData?.studySessions?.activeDays?.length
      ? studentDetailsData.studySessions.activeDays
      : DEFAULT_ACTIVE_DAYS
  const conversationsByWeek =
    studentDetailsData?.lia?.conversationsByWeek?.length
      ? studentDetailsData.lia.conversationsByWeek.map((week, index) => ({
  ...
  const conversationTopics =
    studentDetailsData?.lia?.conversationTopics?.length
  ...
  const topMetrics: MetricCard[] = [
    ...
    },
    {
      icon: TrendingUp,
      label: 'Racha de Dias',
      value: `${studentDetailsData?.studySessions?.studyStreak || 0} dias`,
  ...
  const sofiaMetrics: MetricCard[] = [
    ...
    },
    {
      ...
      sublabel: `Promedio: ${studentDetailsData?.lia?.avgMessagesPerConversation || 0} por conversacion`,
      gradient: COURSE_MANAGEMENT_STAT_GRADIENT_CLASSES.accent,
    },
    {
      icon: Star,
  ...
  const studyMetrics: MetricCard[] = [
    {
      ...
      sublabel: studentDetailsData?.studySessions?.lastSession?.hoursAgo
        ? `Ultima: Hace ${studentDetailsData.studySessions.lastSession.hoursAgo} horas`
        : 'Sin sesiones',
      gradient: COURSE_MANAGEMENT_STAT_GRADIENT_CLASSES.success,
    },
    {
    ...
    },
    {
      ...
      value: `${studentDetailsData?.studySessions?.weeklyFrequency || 0} dias`,
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/80">
                      {selectedStudent.email || selectedStudent.username}
                    </p>
                  </div>
                </div>
                <button
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs leading-relaxed ${COURSE_MANAGEMENT_MUTED_TEXT_CLASS}`}>
                          {buildCourseManagementStudentInsight(studentDetailsData.studySessions)}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className={`p-4 ${COURSE_MANAGEMENT_PANEL_SURFACE_CLASS}`}>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className={`font-semibold ${COURSE_MANAGEMENT_PRIMARY_TEXT_CLASS}`}>{getCourseManagementEnrollmentStatusLabel(selectedStudent.enrollment_status)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className={COURSE_MANAGEMENT_MUTED_TEXT_CLASS}>Inscrito:</span>
                          <span className={`font-semibold ${COURSE_MANAGEMENT_PRIMARY_TEXT_CLASS}`}>{formatDate(selectedStudent.enrolled_at)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className={COURSE_MANAGEMENT_MUTED_TEXT_CLASS}>Ultima Actividad:</span>
                          <span className={`font-semibold ${COURSE_MANAGEMENT_PRIMARY_TEXT_CLASS}`}>{selectedStudent.last_accessed_at ? new Date(selectedStudent.last_accessed_at).toLocaleDateString('es-ES') : 'Nunca'}</span>
                        </div>
                      </div>
                    </div>
