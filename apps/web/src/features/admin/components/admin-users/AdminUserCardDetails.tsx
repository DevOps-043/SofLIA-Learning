  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-center justify-between gap-3 rounded-2xl px-3 py-2" style={{ backgroundColor: theme.inputBg }}>
        <span className="flex min-w-0 items-center gap-2 text-[11px] font-bold uppercase tracking-wider" style={{ color: theme.mutedTextColor }}>
          <Mail className="h-3.5 w-3.5 shrink-0" />
          {t('users.page.table.email')}
        </span>
        <span className="truncate text-right text-xs font-semibold" style={{ color: email ? theme.textColor : theme.mutedTextColor }} title={email || t('users.page.noEmail')}>{email || t('users.page.noEmail')}</span>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 rounded-2xl px-3 py-2" style={{ backgroundColor: theme.inputBg }}>
        <span className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider" style={{ color: theme.mutedTextColor }}>
          <CalendarClock className="h-3.5 w-3.5" />
          {t('users.page.table.lastAccess')}
        </span>
        <span className="truncate text-right text-xs font-semibold" style={{ color: theme.textColor }}>{lastAccess}</span>
      </div>
    </div>
  )
}
