    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-base font-bold text-primary dark:text-white">Preguntas del Quiz</h4>
        </div>
        <motion.button
          type="button"
          onClick={addQuestion}
          ...
          whileTap={{ scale: 0.95 }}
          className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-primary to-primary/90 hover:from-primary hover:to-primary text-white rounded-lg text-sm font-medium transition-all duration-200 shadow-md hover:shadow-lg"
        >
      ...
      <AnimatePresence>
        {questions.map((question, index) => (
          <motion.div
            ...
            className="p-5 bg-white dark:bg-carbon-950 rounded-xl border border-gray-200 dark:border-gray-500/30 hover:border-accent/30 transition-all duration-200"
          >
            {/* Header de la pregunta */}
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200 dark:border-gray-500/30">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-accent/10 dark:bg-accent/20 flex items-center justify-center">
                  <span className="text-sm font-bold text-accent">{index + 1}</span>
                </div>
                <span className="text-sm font-semibold text-primary dark:text-white">Pregunta {index + 1}</span>
              </div>
              <motion.button
                type="button"
            ...
            <div className="mb-4 group">
              ...
              <div className="relative">
                <FileQuestion className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-500 dark:text-white/60 group-focus-within:text-accent transition-colors" />
                <select
            ...
            {question.questionType === 'multiple_choice' && (
              <div className="mb-4">
                <label className="block text-xs font-semibold text-gray-500 dark:text-white/70 mb-2 uppercase tracking-wide">
                  Opciones
                </label>
                <div className="space-y-2">
                      ...
                      <motion.div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-lg border transition-all duration-200 ${
                          isCorrect
                            ...
                            : 'bg-gray-200/30 dark:bg-carbon-950 border-gray-200 dark:border-gray-500/30 hover:border-accent/30'
                        }`}
                      >
                        <button
                          type="button"
                          ...
                          style={{
                            borderColor: isCorrect ? 'var(--color-accent)' : 'var(--color-gray-500)',
                            backgroundColor: isCorrect ? 'var(--color-accent)' : 'transparent'
                          }}
                        >
                          {isCorrect && (
                            <motion.div
                        ...
                        <input
                          ...
                          className="flex-1 px-3 py-2 bg-transparent border-none text-primary dark:text-white placeholder-gray-500 dark:placeholder-white/60 focus:outline-none focus:ring-0"
                        />
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-accent hover:text-accent hover:bg-accent/10 dark:hover:bg-accent/20 rounded-lg transition-all duration-200 border border-accent/20 dark:border-accent/30"
                  >
                    <Plus className="w-4 h-4" />
            ...
            {(question.questionType === 'true_false' || question.questionType === 'short_answer') && (
              <div className="mb-4 group">
                ...
                {question.questionType === 'true_false' ? (
                  <div className="relative">
                    <CheckCircle2 className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-500 dark:text-white/60 group-focus-within:text-accent transition-colors" />
                    <select
      ...
      {questions.length === 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-center py-12 px-4 border-2 border-dashed border-gray-200 dark:border-gray-500/30 rounded-xl bg-gray-200/30 dark:bg-carbon-950"
        >
          <FileQuestion className="w-12 h-12 mx-auto mb-3 text-gray-500 dark:text-white/40" />
