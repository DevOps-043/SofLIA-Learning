...
async function compactLearningPathPositions(learningPathId: string) {
  ...
  if (error) {
    logger.error('Error reloading remaining learning path items:', error)
...
export async function removeLearningPathItem(learningPathId: string, itemId: string) {
  const supabase = createAdminClient()
  const { data: item, error: itemError } = await fromLoose<LearningPathItemRow>(
    supabase,
    'learning_path_items',
  )
    .select('id, learning_path_id, course_id, position')
  ...
  if (!item) {
    throw new Error('Item del learning path no encontrado')
  }
  ...
  await compactLearningPathPositions(learningPathId)
}
