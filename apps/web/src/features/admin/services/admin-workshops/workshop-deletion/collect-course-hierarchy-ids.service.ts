import { collectConversationIds } from './collect-conversation-ids.service'
...
export async function collectCourseHierarchyIds(
  ...
  const moduleIds = await selectIdsByEq(
    supabase,
    'course_modules',
    'module_id',
    'course_id',
  ...
  const lessonIds = await selectIdsByIn(
    supabase,
    'course_lessons',
    'lesson_id',
    'module_id',
  ...
  const materialIds = await selectIdsByIn(
    supabase,
    'lesson_materials',
    'material_id',
    'lesson_id',
  ...
  const activityIds = await selectIdsByIn(
    supabase,
    'lesson_activities',
    'activity_id',
    'lesson_id',
