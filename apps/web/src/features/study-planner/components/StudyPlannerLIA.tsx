...
export function StudyPlannerSofLIA({ onBack }: { onBack?: () => void }) {
  ...
  // Estados para conversación (isListening, transcript, isSpeaking, isAudioEnabled -> useVoiceEngine)
  ...
  // stopAllAudio -> moved to useVoiceEngine hook
...
8. Usa markdown para dar formato (negritas, listas con viñetas, etc.)`;
  ...
  // Voice engine (ELEVENLABS_CONFIG, numberToWords, formatTextForTTS, speakText,
  //    speech recognition init, toggleListening) -> moved to useVoiceEngine hook
  ...
  const handleVoiceQuestion = async (question: string) => {
    ...
    // Deduplicación: el hook ya maneja dedup de transcripts internamente.
    // Aquí solo verificamos si el último mensaje del usuario es idéntico.
    const lastUserMsg = conversationHistoryRef.current.slice().reverse().find(m => m.role === 'user');
    if (lastUserMsg) {
      const lastText = lastUserMsg.content || '';
      if (lastText === question || lastText.includes(question) || question.includes(lastText)) {
...
(Por ejemplo: "Lunes, miércoles y viernes por la noche" o "Fines de semana por la mañana")`;
        ...
        try {
          ...
          if (syncResponse.ok && syncResult.success) {
            ...
            if (syncResponse.status === 401) {
              ...
              setConversationHistory(prev => [...prev, {
                role: 'assistant',
                content: '?? Tu conexión con el calendario ha expirado. Reconecta tu calendario para sincronizar.'
              }]);
            }
          }
  ...
  const toggleAudio = () => {
    voice.toggleAudio(STUDY_PLANNER_STEPS[currentStep].speech);
  };
