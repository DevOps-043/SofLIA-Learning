...
export async function connectGoogleCalendar(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `EMAIL_MISMATCH: El calendario conectado pertenece a "${calendarUserEmail}" pero estás logueado como "${expectedEmail}". Por favor, inicia sesión en Google con la cuenta correcta o cierra la sesión de Google y vuelve a intentar.`,
      )
    }

    const integration = await CalendarDbService.saveCalendarIntegration(
      userId,
      'google',
      tokens,
      calendarUserEmail,
    ...
    if (integration) {
      const secondaryCalendarId = await CalendarGoogleService.getOrCreatePlatformCalendar(
        tokens.access_token,
      )
...
export async function connectMicrosoftCalendar(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `EMAIL_MISMATCH: El calendario conectado pertenece a "${calendarUserEmail}" pero estás logueado como "${expectedEmail}". Por favor, inicia sesión en Microsoft con la cuenta correcta o cierra la sesión y vuelve a intentar.`,
      )
    }

    return await CalendarDbService.saveCalendarIntegration(
      userId,
      'microsoft',
      tokens,
      calendarUserEmail,
