...
describe('selectStudyPlannerFinalSlots — targetWindow date cutoff', () => {
  it('excludes slots after the target date for b2b users', () => {
    const targetDate = ld(2025, 6, 5)
    const slot = makeFreeSlot(ld(2025, 6, 10, 9, 0), ld(2025, 6, 10, 11, 0))
    const day = makeDay(2025, 6, 10, [slot])
    const result = selectStudyPlannerFinalSlots(
      makeInput([day], {
        userType: 'b2b',
        targetWindow: makeTargetWindow({ targetDateObj: targetDate }),
      }),
    )
    expect(result.finalSlots).toHaveLength(0)
  })

  it('includes slots before the target date', () => {
    const targetDate = ld(2025, 6, 20)
    const slot = makeFreeSlot(ld(2025, 6, 4, 9, 0), ld(2025, 6, 4, 11, 0))
    const day = makeDay(2025, 6, 4, [slot])
    const result = selectStudyPlannerFinalSlots(
      makeInput([day], {
...
describe('selectStudyPlannerFinalSlots — non-overlapping selection', () => {
  it('does not return overlapping slots on the same day', () => {
    const slot1 = makeFreeSlot(ld(2025, 6, 4, 9, 0), ld(2025, 6, 4, 11, 0))
    const slot2 = makeFreeSlot(ld(2025, 6, 4, 10, 0), ld(2025, 6, 4, 12, 0)) // overlaps slot1
    const day = makeDay(2025, 6, 4, [slot1, slot2])
    const result = selectStudyPlannerFinalSlots(makeInput([day]))
...
// ─── divideLongSlots with approach ───────────────────────────────────────────

describe('selectStudyPlannerFinalSlots — divideLongSlots', () => {
  it('splits a very long slot for "largo" approach into smaller pieces', () => {
    // 360-minute slot 07:00–13:00
    const slot = makeFreeSlot(ld(2025, 6, 4, 7, 0), ld(2025, 6, 4, 13, 0))
    const day = makeDay(2025, 6, 4, [slot])
    const result = selectStudyPlannerFinalSlots(
      makeInput([day], {
        studyApproach: 'largo' as Parameters<typeof selectStudyPlannerFinalSlots>[0]['studyApproach'],
        profileAvailability: { weeklyMinutes: 300, recommendedSessionLength: 45, recommendedBreak: 10 },
      }),
    )
    // All returned slots should have durationMinutes <= 360
    expect(result.finalSlots.every((s) => s.durationMinutes <= 360)).toBe(true)
  })
})

// ─── weeklyAvailableMinutes ───────────────────────────────────────────────────

describe('selectStudyPlannerFinalSlots — weeklyAvailableMinutes', () => {
  it('computes weeklyAvailableMinutes > 0 when there are valid slots', () => {
    const slot = makeFreeSlot(ld(2025, 6, 4, 9, 0), ld(2025, 6, 4, 11, 0)) // 120 min
    const day = makeDay(2025, 6, 4, [slot])
    const result = selectStudyPlannerFinalSlots(
      makeInput([day], { targetWindow: makeTargetWindow({ weeksUntilTarget: 1 }) }),
    )
    expect(result.weeklyAvailableMinutes).toBeGreaterThan(0)
  })
})

// ─── hasOrganizationalDeadlines preserves all slots ──────────────────────────

describe('selectStudyPlannerFinalSlots — organizational deadlines', () => {
  it('skips distribution reduction when hasOrganizationalDeadlines is true', () => {
    const days = [4, 5, 6, 7, 8, 9, 10].map((d) => {
      const slot = makeFreeSlot(ld(2025, 6, d, 9, 0), ld(2025, 6, d, 11, 0))
      return makeDay(2025, 6, d, [slot])
    })

    const withDeadline = selectStudyPlannerFinalSlots(
      makeInput(days, { hasOrganizationalDeadlines: true, totalLessonsNeeded: 5 }),
    )
    const noDeadline = selectStudyPlannerFinalSlots(
      makeInput(days, { hasOrganizationalDeadlines: false, totalLessonsNeeded: 5 }),
    )

    // With deadline: all slots preserved; without: distribution may reduce them
    expect(withDeadline.finalSlots.length).toBeGreaterThanOrEqual(noDeadline.finalSlots.length)
  })
})

// ─── Consistent dateStr on returned slots ────────────────────────────────────

describe('selectStudyPlannerFinalSlots — slot metadata', () => {
  it('returned slots carry dateStr and dayName from the source day', () => {
    const slot = makeFreeSlot(ld(2025, 6, 4, 9, 0), ld(2025, 6, 4, 11, 0))
    const day = makeDay(2025, 6, 4, [slot], { dayName: 'Miércoles' })
    const result = selectStudyPlannerFinalSlots(makeInput([day]))
    expect(result.finalSlots.length).toBeGreaterThan(0)
    result.finalSlots.forEach((s) => {
      expect(s.dateStr).toBe(dateKey(2025, 6, 4))
      expect(s.dayName).toBe('Miércoles')
    })
  })
})
