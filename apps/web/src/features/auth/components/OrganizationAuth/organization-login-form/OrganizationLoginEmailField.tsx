...
export function OrganizationLoginEmailField(props: OrganizationLoginFieldProps) {
  ...
  return (
    <motion.div className="space-y-1.5" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.4 }}>
      ...
      <motion.div className="relative rounded-xl border transition-all duration-300 overflow-hidden" style={{ backgroundColor: props.palette.inputBgColor, borderColor: focused ? props.palette.focusColor : props.errors.emailOrUsername ? '#ef4444' : props.palette.borderColor, borderWidth: focused ? '2px' : '1px', boxShadow: focused ? `0 0 0 3px ${props.palette.focusColor}20, 0 4px 12px -2px rgba(0, 0, 0, 0.2)` : 'none' }} animate={{ scale: focused ? 1.005 : 1 }} transition={{ duration: 0.2 }}>
        ...
        <div className="flex items-center px-4 py-3">
          <Mail className="w-4 h-4 flex-shrink-0 mr-3 transition-colors duration-200" style={{ color: focused ? props.palette.focusColor : `${props.palette.textColor}50` }} />
