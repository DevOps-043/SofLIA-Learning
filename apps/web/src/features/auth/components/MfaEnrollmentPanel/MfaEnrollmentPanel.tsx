...
export function MfaEnrollmentPanel({ backHref }: MfaEnrollmentPanelProps) {
  ...
  async function loadStatus() {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : t('auth.mfa.enrollment.errors.status'),
        'error',
      )
    } finally {
      setIsLoading(false)
  ...
  async function startSetup() {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : t('auth.mfa.enrollment.errors.setup'),
        'error',
      )
    } finally {
      setIsWorking(false)
  ...
  async function activateMfa() {
    ...
    try {
      ...
      showToast(t('auth.mfa.enrollment.activated'), 'success')
      setSetup(null)
      setActivationToken('')
      await loadStatus()
    } catch (error) {
      showToast(
        error instanceof Error ? error.message : t('auth.mfa.enrollment.errors.activate'),
        'error',
      )
    } finally {
      setIsWorking(false)
  ...
  async function copyRecoveryCodes() {
    ...
    showToast(t('auth.mfa.enrollment.recoveryCopied'), 'success')
  }
