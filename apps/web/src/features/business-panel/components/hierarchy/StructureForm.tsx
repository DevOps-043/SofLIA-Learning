                        ...
                        <div className="flex min-h-[400px]">
                            ...
                            <div className="hidden md:flex w-64 p-8 flex-col items-center justify-center space-y-6 border-r border-neutral-100 dark:border-white/5 bg-neutral-50/50 dark:bg-black/10">
                                <motion.div
                                    ...
                                    className="w-20 h-20 rounded-[2rem] flex items-center justify-center shadow-xl"
                                    style={{
                                        background: `linear-gradient(135deg, ${theme.actionColor}, ${theme.secondaryColor})`,
                                    }}
                                >
                                    <Layout className="w-10 h-10" style={{ color: theme.onActionColor }} />
                                </motion.div>
                                <div className="text-center space-y-2">
                                    <div className="flex items-center justify-center gap-2">
                                        <Sparkles className="w-3 h-3" style={{ color: theme.actionColor }} />
                                        <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: theme.actionColor }}>{t('hierarchy.structureForm.sideTitle')}</span>
                                    </div>
                            ...
                            <div className="flex-1 p-8 flex flex-col">
                                <div className="flex items-center justify-between mb-8">
                                    <h3 className="text-xl font-black tracking-tight italic" style={{ color: theme.actionColor }}>{t('hierarchy.structureForm.title')}</h3>
                                ...
                                <form onSubmit={handleSubmit} className="flex-1 flex flex-col space-y-6">
                                    <div className="space-y-4">
                                        ...
                                        <div className="relative group">
                                            <Plus className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-300 dark:text-white/10 transition-colors" />
                                            <input
                                                type="text"
                                                value={name}
                                                onChange={(e) => setName(e.target.value)}
                                                placeholder={t('hierarchy.structureForm.placeholders.name')}
                                                className="w-full pl-12 pr-6 py-4 rounded-2xl border-2 bg-white dark:bg-black/20 border-neutral-100 dark:border-white/5 text-sm font-bold outline-none transition-all placeholder:text-neutral-300 dark:placeholder:text-white/10"
                                                style={{
                                    ...
                                    <div className="flex items-center gap-3 pt-6 border-t border-neutral-100 dark:border-white/5">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl disabled:opacity-50 active:scale-95"
                                            style={{
