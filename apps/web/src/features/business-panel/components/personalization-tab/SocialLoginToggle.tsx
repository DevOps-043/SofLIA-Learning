...
export function SocialLoginToggle({
  ...
  return (
    <motion.div className="flex items-center justify-between p-4 rounded-xl border transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/5 bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/10" whileHover={{ x: 2 }}>
      ...
      <motion.button type="button" onClick={() => void onToggle()} disabled={isUpdating} whileTap={{ scale: 0.95 }} className="relative inline-flex h-8 w-16 items-center rounded-full transition-all duration-300 focus:outline-none shadow-inner" style={{ background: enabled ? 'linear-gradient(135deg, var(--color-success), var(--color-legacy-059669))' : 'rgba(156, 163, 175, 0.3)' }}>
