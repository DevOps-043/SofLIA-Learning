        ...
        {links.map((item, index) => (
          <motion.div key={item.type} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * index }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <motion.button type="button" onClick={() => state.copyToClipboard(item.url, item.type)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={`px-4 py-3 rounded-xl transition-all duration-300 flex items-center gap-2 whitespace-nowrap shadow-lg ${item.copied ? 'bg-success text-white' : ''}`} style={item.copied ? undefined : { backgroundColor: theme.actionColor, color: theme.onActionColor }}>
