...
export function InviteLinkActionMenu({ link, menuRef, onAction, openMenuId }: InviteLinkActionMenuProps) {
  ...
  return (
    <AnimatePresence>
      {openMenuId === link.id && (
        <motion.div ref={menuRef} initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} className="absolute right-0 top-full mt-1 w-40 rounded-xl border shadow-lg overflow-hidden" style={{ backgroundColor: theme.isDark ? 'var(--color-legacy-252b3b)' : 'var(--color-bg-light)', borderColor: theme.borderColor, zIndex: 50 }}>
          {link.status === 'active' && <MenuButton icon={<Pause className="w-4 h-4" style={{ color: 'var(--color-warning)' }} />} label={t('users.modals.manageLinks.actions.pause', 'Pausar')} onClick={() => onAction(link.id, 'pause')} />}
          {link.status === 'paused' && <MenuButton icon={<Play className="w-4 h-4" style={{ color: 'var(--color-legacy-22c55e)' }} />} label={t('users.modals.manageLinks.actions.resume', 'Reanudar')} onClick={() => onAction(link.id, 'resume')} />}
