...
export function BusinessAssignCourseFooter({ modal, t, theme }: BusinessAssignCourseFooterProps) {
  const removalOnly = modal.pendingRemovalIds.size > 0 && modal.selectedUserCount === 0;
  ...
  return (
    <div className="shrink-0 p-5 px-8 flex items-center justify-between gap-4 border-t" style={{ backgroundColor: theme.panelBg, borderColor: theme.borderColor }}>
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto">
        <button type="button" onClick={modal.handleClose} className="flex-1 sm:flex-none px-6 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all" style={{ color: theme.mutedTextColor, backgroundColor: theme.inputBg, borderColor: theme.borderColor }}>{t('users.buttons.cancel')}</button>
        <motion.button
...
function getConfirmLabel(modal: BusinessAssignCourseModalState, t: TFunction<'business'>): string {
  if (modal.pendingRemovalIds.size > 0 && modal.selectedUserCount === 0) return `Confirmar (${modal.pendingRemovalIds.size} a quitar)`;
  if (modal.pendingRemovalIds.size > 0) return `Confirmar (${modal.selectedUserCount} asignar · ${modal.pendingRemovalIds.size} quitar)`;
