export const FALLBACK_TEXT_ES = {
  ...
  noSegment: 'No hay segmentos con evidencia evaluada suficiente para comparar en este periodo.',
  ...
  recommendQuality: 'Revisa las entregas y evaluaciones con resultados bajos, junto con las solicitudes de ayuda, para ajustar instrucciones, ejemplos y criterios.',
  ...
  // Action plan items
  actionPlanSegment: 'Prioriza los segmentos con bajo resultado en evidencia evaluada o bajo avance antes de ampliar nuevas asignaciones.',
  ...
  summary: (quality: number, progress: number, atRisk: number, compliance: number, evidence: number) =>
    `La organizacion registra un progreso promedio del ${progress}%. El ${compliance}% de los colaboradores asignados no presenta senales de riesgo segun la regla de seguimiento. Las ${evidence} evidencias evaluadas promedian ${quality}%${atRisk > 0 ? ` y ${atRisk} colaboradores requieren atencion prioritaria` : ''}.`,
  ...
  metricQualityDetail: (quiz: number, activity: number, evidence: number) =>
    `${evidence} evidencias: evaluaciones ${quiz}% y entregas validadas ${activity}%.`,
  ...
  qualityPoint: (quality: number, evidence: number) =>
    `Las ${evidence} evidencias evaluadas registran un resultado promedio del ${quality}%.`,
  segmentPoint: (label: string, score: number, users: number) =>
    `La franja "${label}" agrupa ${users} ${users === 1 ? 'persona' : 'personas'} y registra un resultado de evidencia evaluada del ${score}%.`,
  ...
  riskOverdue: (count: number) =>
    `${count} asignaciones vencidas acumuladas en el periodo requieren seguimiento operativo.`,
  ...
  urgentAtRiskTitle: 'Colaboradores con senales de riesgo',
  urgentAtRiskDesc: (count: number) =>
    `${count} colaboradores presentan al menos una senal de seguimiento: vencimientos, falta de inicio, inactividad o bajo avance. Requieren un plan de recuperacion y contacto directo en los proximos dias.`,
  urgentAtRiskTimeline: '1-2 dias',
  urgentOverdueTitle: 'Vencimientos acumulados sin resolver',
  urgentOverdueDesc: (count: number) =>
    `${count} ${count === 1 ? 'asignacion vencida detectada' : 'asignaciones vencidas detectadas'} en el periodo. Prioriza contacto con los responsables de area para acordar fechas de recuperacion.`,
  ...
  kudoQualityTitle: 'Buen resultado en evidencia evaluada',
  kudoQualityDesc: (score: number) =>
    `El resultado promedio del ${score}% corresponde exclusivamente a evaluaciones y entregas registradas en el periodo.`,
