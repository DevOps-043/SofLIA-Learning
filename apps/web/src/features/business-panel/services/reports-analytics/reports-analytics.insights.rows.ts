import type { ReportsAnalyticsDataset, ReportsAnalyticsLocale } from '../../types/reports-analytics.types'

export function buildInsightSegmentRows(dataset: ReportsAnalyticsDataset, labels: Record<string, string>) {
  return [
...
export function buildInsightHierarchyRows(dataset: ReportsAnalyticsDataset) {
  return [...dataset.rankings.regions, ...dataset.rankings.zones, ...dataset.rankings.teams]
    .sort((a, b) => b.rankScore - a.rankScore)
}

export function formatReportsAnalyticsDate(value: string, locale: ReportsAnalyticsLocale): string {
