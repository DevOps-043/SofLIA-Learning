...
import { FALLBACK_TEXT } from './reports-analytics.insights/fallback-text'

export function buildFallbackReportsAnalyticsInsights(
  ...
  return {
    ...
    executiveMetrics: [
      { label: language.metricProgress, value: dataset.overview.averageProgress + '%', detail: language.metricProgressDetail(dataset.overview.completionRate, dataset.learning.medianCompletionDays) },
      { label: language.metricSoflia, value: dataset.overview.sofliaAdoptionRate + '%', detail: language.metricSofliaDetail(dataset.soflia.totalConversations, dataset.soflia.totalMessages) },
      { label: language.metricQuality, value: dataset.quality.overallScore + '%', detail: language.metricQualityDetail(dataset.quality.quizScore, dataset.quality.activityScore, dataset.quality.sofliaScore) },
    ],
    findings: [
      {
    ...
    actionPlan: [{ title: language.actionPlanTitle, points: [language.actionPlanSegment, language.actionPlanCourse, language.actionPlanData] }],
  }
}
