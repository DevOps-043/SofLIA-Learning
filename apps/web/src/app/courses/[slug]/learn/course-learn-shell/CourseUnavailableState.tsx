...
function LearningPathBlockedState({ logic }: { logic: LearnPageLogicResult }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-white px-6 py-12 dark:bg-carbon-900">
      <div className="w-full max-w-2xl rounded-3xl border border-amber-500/20 bg-white p-8 shadow-[0_24px_80px_rgba(10,37,64,0.08)] dark:bg-gray-900">
        ...
        <p className="mt-4 text-sm leading-6 text-[var(--color-legacy-52606d)] dark:text-white/75">{state.message}</p>
        <div className="mt-6 rounded-2xl border border-accent/20 bg-accent/5 p-4">
          ...
          <p className="mt-1 text-xs text-[var(--color-legacy-52606d)] dark:text-white/60">{logic.t('leftPanel.learningPath.completedCount', { completed: learningPath.completedItemsCount, total: learningPath.totalItemsCount })}</p>
