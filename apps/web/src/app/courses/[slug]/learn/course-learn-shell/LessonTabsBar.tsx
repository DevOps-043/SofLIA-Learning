    ...
    <div id={COURSE_LEARN_TOUR_TARGET_IDS.tools} className="scrollbar-hide flex h-[56px] items-center gap-1 overflow-x-auto scroll-smooth rounded-t-xl border-b border-gray-200 bg-white p-2 md:gap-2 md:p-3 dark:border-gray-500/30 dark:bg-carbon-800" style={{ scrollPaddingLeft: '0.5rem', scrollPaddingRight: '0.5rem', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch' }}>
      <div className="flex min-w-max items-center gap-1 md:gap-2">
        {logic.tabs.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => logic.handleTabChange(tab.id)} className={`group relative flex shrink-0 items-center rounded-xl transition-all duration-200 ${shouldHideText ? 'px-2 py-2 hover:gap-2 hover:px-3' : 'min-w-fit gap-1 px-3 py-2 md:gap-2 md:px-4'} ${isActive ? 'bg-primary text-white shadow-lg shadow-primary/25 dark:bg-accent dark:text-primary dark:shadow-accent/25' : 'text-gray-500 hover:bg-gray-200/50 hover:text-primary dark:text-white/60 dark:hover:bg-accent/10 dark:hover:text-accent'}`} style={{ fontFamily: 'Inter, sans-serif', fontWeight: isActive ? 600 : 500, scrollSnapAlign: 'start' }}>
