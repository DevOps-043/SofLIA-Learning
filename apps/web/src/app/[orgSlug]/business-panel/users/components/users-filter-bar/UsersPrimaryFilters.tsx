  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
      ...
      {isUsersTab ? <>
        ...
        <button onClick={() => props.setShowAdvancedFilters(!props.showAdvancedFilters)} className="flex items-center gap-2 rounded-xl border-2 px-4 py-3.5 transition-all duration-300" style={{ backgroundColor: props.showAdvancedFilters ? `color-mix(in srgb, ${theme.primaryColor} 12.5%, transparent)` : theme.cardBg, borderColor: props.showAdvancedFilters || props.activeFiltersCount > 0 ? theme.primaryColor : theme.borderColor, color: theme.textColor }}><Filter className="h-4 w-4" /><span className="hidden text-sm sm:inline">{props.t('users.filters.advanced')}</span>{props.activeFiltersCount > 0 ? <span className="rounded-full px-1.5 py-0.5 text-xs font-bold" style={{ backgroundColor: theme.primaryColor, color: theme.onPrimaryColor }}>{props.activeFiltersCount}</span> : null}</button>
      </> : null}
