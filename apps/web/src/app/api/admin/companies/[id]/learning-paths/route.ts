...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'Error al obtener learning paths de la empresa' },
      { status: 500 },
    )
  }
}
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!body.learningPathId) {
      return NextResponse.json(
        { success: false, error: 'LearningPathId es requerido' },
        { status: 400 },
      )
    }
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await AdminLearningPathsService.revokeFromOrganization(id, assignmentId)
    return NextResponse.json({ success: true })
  } catch (error) {
    logger.error('Error revoking learning path from organization:', error)
    return NextResponse.json(
      { success: false, error: 'Error al revocar learning path de la empresa' },
      { status: 500 },
    )
  }
}
