...
export function buildVerifiedLessonDurationSection(lessonContext: LessonContext): string {
  const totalDurationMinutes =
    typeof lessonContext.totalDurationMinutes === 'number' && lessonContext.totalDurationMinutes > 0
      ? lessonContext.totalDurationMinutes
      : undefined
  const videoDurationMinutes =
    typeof lessonContext.durationSeconds === 'number' && lessonContext.durationSeconds > 0
