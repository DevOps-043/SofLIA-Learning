...
export function buildInteractiveActivitySection(context: PlatformContext): string {
  if (!context.currentActivityContext) return ''

  const activity = context.currentActivityContext
  const effectiveUserJobTitle = resolveEffectiveUserJobTitle(context, context.currentLessonContext)
  let section = '\n### ACTIVIDAD INTERACTIVA EN CURSO (FOCO PRINCIPAL)\n'
  section += `El usuario esta realizando la actividad: "${activity.title}"\n`
  section += `Tipo: ${activity.type}\n`
  section += `Descripcion/Instruccion: ${activity.description}\n`
  ...
  section += buildActivityStrategy(effectiveUserJobTitle)
  section += buildActivityResponseRules()

  if (activity.prompts?.length) {
    section += `- Prompts sugeridos para esta actividad: ${activity.prompts.join(' | ')}\n`
  }

  return section
}

function buildActivityStrategy(effectiveUserJobTitle?: string): string {
  let section = '### ESTRATEGIA DE INTERACCION (APLICAR SIEMPRE):\n'
  ...
  if (effectiveUserJobTitle) {
    section += `El usuario es "${effectiveUserJobTitle}". Usa ejemplos del mundo real aplicables a su cargo y preguntale como llevaria el concepto a su trabajo.\n`
  } else {
...
function buildActivityResponseRules(): string {
  return (
