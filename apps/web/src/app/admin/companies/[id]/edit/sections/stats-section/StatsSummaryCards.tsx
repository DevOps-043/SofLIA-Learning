...
export function StatsSummaryCards({ overview }: { overview: StatsOverview }) {
  return (
    <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
      {ITEMS.map((item) => (
        <motion.div key={item.key} whileHover={{ y: -5 }} className="rounded-2xl border border-gray-100 bg-gray-50 p-5 shadow-lg dark:border-white/5 dark:bg-[#0F1419]">
          ...
          <p className="mt-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 dark:text-[#8899A6]">{item.label}</p>
        </motion.div>
      ))}
    </div>
  )
